...
from aifunctiongenerator_consts import (
    AIFUNCTIONGENERATOR_CONNECTIVITY_ENDPOINT,
    AIFUNCTIONGENERATOR_ERR_CONNECTIVITY_TEST,
    AIFUNCTIONGENERATOR_SUCC_CONNECTIVITY_TEST,
)


class AifunctiongeneratorConnector(BaseConnector):
    """
    Aifunctiongenerator connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = AIFUNCTIONGENERATOR_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(AIFUNCTIONGENERATOR_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(AIFUNCTIONGENERATOR_SUCC_CONNECTIVITY_TEST)
